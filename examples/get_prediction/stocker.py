...
def get_date(stock):
    dates_lst = json.loads(stock.to_json(orient="split"))['index']
    for i in range(len(dates_lst)):
        dates_lst[i] = dates_lst[i].split()[0]
    return dates_lst
...
def get_data_from_alpha_vantage():
    ...
    stock, meta_data = ts.get_intraday(symbol='MSFT', interval='60min',
                                       outputsize='full')
    #
    if ('Adj. Close' not in stock.columns):
        stock['Adj. Close'] = stock['4. close']
        stock['Adj. Open'] = stock['1. open']

    # Dates of stock, example: 2019-03-31
    stock['ds'] = get_date(stock)
...
class Stocker():
    ...
    def __init__(self, ticker, exchange='WIKI'):
        ...
        stock = get_data_from_alpha_vantage()
        print(stock)
        ...
        print(type(self.min_date), type(stock['ds']))
        print('sdfgjukiloifegkild')
        print(self.min_date)
            ...
            while (not valid_start) & (not valid_end):
                ...
                if (start_date not in list(df['Date'])):
                    ...
                    end_date = pd.to_datetime(
                        input(prompt='Enter a new end date: '))
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'],
        ...
        for i, stat in enumerate(stats):
            ...
            print('Maximum {} = {:.2f} on {}.'.format(stat, stat_max,
                                                      date_stat_max))
            print('Minimum {} = {:.2f} on {}.'.format(stat, stat_min,
                                                      date_stat_min))
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock.ix[
                len(self.stock) - 1, stat], self.max_date.date()))
            ...
            if plot_type == 'pct':
                ...
                plt.style.use('fivethirtyeight');
                plt.plot(stock_plot['Date'], stock_plot[stat], color=colors[i],
                         linewidth=3, label=stat, alpha=0.8)
                plt.xlabel('Date');
                plt.ylabel('US $');
                plt.title('%s Stock History' % self.symbol);
                plt.legend(prop={'size': 10})
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        start_price = float(
            self.stock[self.stock['Date'] == start_date]['Adj. Open'])
        end_price = float(
            self.stock[self.stock['Date'] == end_date]['Adj. Close'])

        # Make a profit dataframe and calculate profit column
        profits = self.make_df(start_date, end_date)
        profits['hold_profit'] = nshares * (
                profits['Adj. Close'] - start_price)
    ...
    def changepoint_prior_analysis(self,
        ...
        color_dict = {prior: color for prior, color in
                      zip(changepoint_priors, colors)}

        # Plot each of the changepoint predictions
        for prior in changepoint_priors:
            # Plot the predictions themselves
            ax.plot(predictions['ds'], predictions['%.3f_yhat' % prior],
                    linewidth=1.2,
                    color=color_dict[prior], label='%.3f prior scale' % prior)

            # Plot the uncertainty interval
            ax.fill_between(predictions['ds'].dt.to_pydatetime(),
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        if days > 0:
            ...
            print('Predicted Price on {} = ${:.2f}'.format(
                future.ix[len(future) - 1, 'ds'].date(),
        ...
        ax.plot(stock_history['ds'], stock_history['y'], 'ko-', linewidth=1.4,
                alpha=0.8, ms=1.8, label='Observations')

        # Plot the predicted values
        ax.plot(future['ds'], future['yhat'], 'forestgreen', linewidth=2.4,
                label='Modeled');

        # Plot the uncertainty interval as ribbon
        ax.fill_between(future['ds'].dt.to_pydatetime(), future['yhat_upper'],
                        future['yhat_lower'], alpha=0.3,
                        facecolor='g', edgecolor='k', linewidth=1.4,
    ...
    def evaluate_prediction(self, start_date=None, end_date=None,
                            nshares=None):
        ...
        train = self.stock[(self.stock['Date'] < start_date.date()) &
                           (self.stock['Date'] > (start_date - pd.DateOffset(
                               years=self.training_years)).date())]
        ...
        test['correct'] = (np.sign(test['pred_diff']) == np.sign(
            test['real_diff'])) * 1

        # Accuracy when we predict increase and decrease
        increase_accuracy = 100 * np.mean(
            test[test['pred_diff'] > 0]['correct'])
        decrease_accuracy = 100 * np.mean(
            test[test['pred_diff'] < 0]['correct'])
        ...
        for i in test.index:
            if (test.ix[i, 'y'] < test.ix[i, 'yhat_upper']) & (
                    test.ix[i, 'y'] > test.ix[i, 'yhat_lower']):
        ...
        if not nshares:
            ...
            # Plot the actual values
            ax.plot(train['ds'], train['y'], 'ko-', linewidth=1.4, alpha=0.8,
                    ms=1.8, label='Observations')
            ax.plot(test['ds'], test['y'], 'ko-', linewidth=1.4, alpha=0.8,
                    ms=1.8, label='Observations')

            # Plot the predicted values
            ax.plot(future['ds'], future['yhat'], 'navy', linewidth=2.4,
                    label='Predicted');
            ...
            ax.fill_between(future['ds'].dt.to_pydatetime(),
                            ...
                            alpha=0.6,
                            facecolor='gold', edgecolor='k', linewidth=1.4,
                            label='Confidence Interval')

            # Put a vertical line at the start of predictions
            plt.vlines(x=min(test['ds']).date(),
        ...
        # If a number of shares is specified, play the game
            ...
            test = pd.merge(test, test_pred_increase[['ds', 'pred_profit']],
                            on='ds', how='left')
    ...
    def retrieve_google_trends(self, search, date_range):
        ...
        try:
            ...
            pytrends.build_payload(kw_list, cat=0, timeframe=date_range[0],
                                   geo='', gprop='news')
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        train = self.stock[self.stock['Date'] > (self.max_date - pd.DateOffset(
            years=self.training_years)).date()]
        model.fit(train)
        ...
        if not search:
            ...
            plt.plot(future['ds'], future['yhat'], color='navy', linewidth=2.0,
                     label='Modeled')

            # Changepoints as vertical lines
            plt.vlines(cpos_data['ds'].dt.to_pydatetime(),
                       ymin=min(train['y']), ymax=max(train['y']),
                       linestyles='dashed', color='r',
                       linewidth=1.2, label='Negative Changepoints')

            plt.vlines(cneg_data['ds'].dt.to_pydatetime(),
                       ymin=min(train['y']), ymax=max(train['y']),
        ...
        if search:
            date_range = ['%s %s' % (
                str(min(train['Date']).date()),
                str(max(train['Date']).date()))]

            # Get the Google Trends for specified terms and join to training dataframe
            trends, related_queries = self.retrieve_google_trends(search,
                                                                  date_range)
            ...
            plt.plot(train['ds'], train['freq_norm'], color='goldenrod',
            ...
            plt.title('%s Stock Price and Search Frequency for %s' % (
                self.symbol, search))
            plt.show()
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None,
        ...
        train = self.stock[(self.stock['Date'] > (start_date - pd.DateOffset(
            years=self.training_years)).date()) &
        ...
        results = pd.DataFrame(0, index=list(range(len(changepoint_priors))),
                               columns=['cps', 'train_err', 'train_range',
                                        'test_err', 'test_range'])
        ...
        for i, prior in enumerate(changepoint_priors):
            ...
            train_results = pd.merge(train, future[
                ['ds', 'yhat', 'yhat_upper', 'yhat_lower']], on='ds',
                                     how='inner')
            avg_train_error = np.mean(
                abs(train_results['y'] - train_results['yhat']))
            ...
            test_results = pd.merge(test, future[
                ['ds', 'yhat', 'yhat_upper', 'yhat_lower']], on='ds',
                                    how='inner')
            avg_test_error = np.mean(
                abs(test_results['y'] - test_results['yhat']))
        ...
        plt.plot(results['cps'], results['train_err'], 'bo-', ms=8,
                 label='Train Error')
        plt.plot(results['cps'], results['test_err'], 'r*-', ms=8,
                 label='Test Error')
        ...
        plt.plot(results['cps'], results['train_range'], 'bo-', ms=8,
                 label='Train Range')
        plt.plot(results['cps'], results['test_range'], 'r*-', ms=8,
                 label='Test Range')
        plt.xlabel('Changepoint Prior Scale');
